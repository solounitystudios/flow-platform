...
export function PersonCard({ person, className }: { person: Pick<MockPerson, "id" | "username" | "full_name" | "avatar_url" | "city" | "state" | "bio" | "reliability_score" | "available_now">; className?: string }) {
  return (
    <Link
      href={`/p/${person.username}`}
      className={cn(
        "flex items-center gap-3 rounded-2xl border border-ink-100 bg-white p-3.5 transition hover:border-flow-300 hover:shadow-card dark:border-ink-800 dark:bg-ink-900",
        className,
      )}
    >
