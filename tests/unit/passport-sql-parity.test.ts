...
import { CLAIM_TRANSITIONS, VERIFICATION_METHOD_POLICY } from "@/lib/passport/domain";
...
function quotedListAfter(anchor: string): string[] {
...
describe("SQL <-> contracts vocabulary parity", () => {
  ...
  it("claim statuses", () => expect(quotedListAfter("status text not null default 'draft' check (status in (")).toEqual(sorted(CLAIM_STATUSES)));
  ...
  it("evidence statuses", () => expect(quotedListAfter("status text not null default 'received' check (status in (")).toEqual(sorted(EVIDENCE_STATUSES)));
  ...
  it("authority statuses", () => expect(quotedListAfter("status text not null default 'active' check (status in (")).toEqual(sorted(AUTHORITY_STATUSES)));
