-- get_or_create_event_conversation already denies a cancelled attendee a
-- NEW join, but conversation_members is a separate persistent row: anyone
-- who joined the event chat before cancelling stayed a member afterward,
-- since RLS (is_conversation_member) only checks conversation_members, not
-- current attendance status. A cancelled attendee who had joined chat kept
-- full read/write access to it indefinitely.
--
-- Fix: when enforce_attendance_lifecycle processes a registered/attended ->
-- cancelled transition, also delete that profile's conversation_members row
-- for the event's conversation (if one exists). This is SECURITY DEFINER,
-- so it applies regardless of the caller's own RLS — the caller only has
-- UPDATE rights on their own event_attendance row via attendance_self_cancel;
-- this cleanup is a system-owned side effect of that transition, not a new
-- privilege granted to the client.
--
-- Reactivation (cancelled -> registered) does not need a symmetric re-add:
-- get_or_create_event_conversation already re-admits an eligible attendee
-- the next time they open the event chat, since eligibility is
-- re-evaluated fresh on every call.
